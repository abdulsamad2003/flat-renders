...
const Flats = () => {
  ...
  const openShapesparkView = (flat) => {
    ...
    window.history.pushState(
      {},
      "",
      `/flat/${flat.shapespark_url.toLowerCase()}`
    );
  };
  ...
  const [floorRange, setFloorRange] = useState([1, 6]);
  ...
  const getFlatClass = (id) => {
    ...
    const matchesBHK =
      selectedBHKs.length === 0 || selectedBHKs.includes(flat.bhk);
    const matchesArea = flat.area >= areaRange[0] && flat.area <= areaRange[1];
    const matchesFloor =
      ...
      <article className="wrapper-container">
        <section className="wrapper" ref={scrollableRef}>
          ...
          {selectedFlats && (
            ...
            >
              <div className="close-btn" onClick={() => setSelectedFlats(null)}>
                {/* Close button */}
                <IoMdClose className="close-icon" />
              </div>
              {!isImageLoaded && (
                <div className="floor-image-loading">
                  <span className="loader main-font">Loading image...</span>
                </div>
              )}
              <img
        ...
        {!isFilterComponentVisible ? (
          <div className="filter-btn">
            <button
              className="main-font secondary-btn"
